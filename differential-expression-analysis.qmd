...
```{r}
plot_parallel2(logcounts, main = "10% Most Variable Genes")
```
...
NOTE: `edgeR::plotMDS(y)` is also a good alternative to PCA. 
...
# MDS (similar to PCA)
plotMDS(lcpms, col = y$samples$group, pch = 16, cex = 2)
legend("top", legend = levels(y$samples$group), col = palette()[1:2], pch = 16)
```
...
```{r, eval=FALSE}
# edgeR 4.0 workflow (recommended)
...
Over-representation testing can be performed using the `goseq` package which 
incorporates length bias inherent in RNA-seq when performing over-representation
testing. Using `goseq` requires gene length information. `goseq` has several 
gene length datasets built-in, however, when gene length data is not available 
you may need to compute it yourself (as in the case of GENCODE v48 annotations, 
for example). 

Below I will show how you can compute gene length information and use that in 
`goseq` analysis. There are two ways to think about computing gene lengths, 
either taking the median transcript length across transcripts of a given gene or
by taking the total length of all reduced exons. Either should suffice and I'm 
not sure if one is better than the other. For reference, `goseq` uses the median 
transcript length. 

### Computing gene length information

**The annotation used in the airway package is from Ensembl v75** we can import
the gtf as a txdb object in order to compute gene length information
...
Compute the reduced exon lengths
...
And compute the median transcript lengths
...
With this information we are ready to perform over-representation testing with 
`goseq`. 

### Perform over-representation testing with `goseq`

```{r, warning=FALSE, message=FALSE}
library(goseq)


# First create an indicator vector for the significant genes
up_genes <- de_result$logFC > 0 & de_result$FDR < 0.05
down_genes <- de_result$logFC < 0 & de_result$FDR < 0.05

# Name each set with the ENSEMBL IDs
names(up_genes) <- names(down_genes) <- de_result$gene_id
```

Next, Compute the probability weighted functions for each set of up and 
down-regulated genes. **NOTE** `goseq` has built-in genomes and annotations
that can be used in place of the pre-computed lengths above, I just wanted to 
show how you might compute these lengths if needed (especially since newer 
annotation version are not present). The pwf plots should generally increase
with increasing length.

```{r, warning=FALSE, message=FALSE}
up_pwf <- suppressWarnings(
  nullp(
    up_genes,
    genome = "hg19",
    id = "ensGene",
    bias.data = reduced_exon_lengths[names(up_genes)],
    plot.fit = TRUE
  )
)

down_pwf <- suppressWarnings(
  nullp(
    down_genes,
    genome = "hg19",
    id = "ensGene",
    bias.data = reduced_exon_lengths[names(down_genes)],
    plot.fit = TRUE
  )
)
```

Finally, perform over-representation testing

```{r, warning=FALSE, message=FALSE}
# Perform GO analysis
up_go <- goseq(up_pwf, genome = "hg19", id = "ensGene")
down_go <- goseq(down_pwf, genome = "hg19", id = "ensGene")

# Add multiple test correction
up_go$FDR <- p.adjust(up_go$over_represented_pvalue, method = "BH")
down_go$FDR <- p.adjust(down_go$over_represented_pvalue, method = "BH")
```

We can show these results using a dotplot. Below is a simple function used to 
create gene ontology dotplots from the top 15 significant go terms. 

```{r}
plot_dotplot <- function(df, n = 15) {
  df <- df[order(df$FDR), ]
  df <- head(df, n = n)
  df$prop_in_set <- df$numDEInCat / df$numInCat

  ggplot(df, aes(x = -log10(FDR), y = reorder(term, -FDR))) +
    geom_point(aes(size = prop_in_set, color = -log10(FDR))) +
...
plot_dotplot(up_go) + labs(title = "Up-regulated Genes")
plot_dotplot(down_go) + labs(title = "Down-regulated Genes")
```
