...
in the paper, ["All biology is computational biology"](https://journals.plos.org/plosbiology/article?id=10.1371/journal.pbio.2002050) by Florian Markowetz. Data is getting bigger and bigger and we need to be equipped
...
**Pro-tip** one nice way of iterating over files that do have spaces in their names is outlined
in this [stackoverflow comment](https://stackoverflow.com/a/7039208). 

```{.bash}
# Source - https://stackoverflow.com/a/7039208
# Posted by martin clayton
...
`subject1.fq.fz` and the files from the ATAC-seq experiment `control_subject1.fq.gz` 
if they refer to the same sample. For small projects, it's fairly easy to create consistent 
and unique IDs for each subject. For large projects unique random IDs can be used.
...
If you have a large study and you need to create unique paticipant IDs there are 
[several common methods](https://cghlewis.com/blog/participant_id/). My prefered option is the 
create a completely unique ID devoid of any personally identifying information using a simple study 
identifier and a number. This identifier stays with that participant across any new experiments. 
Metadata is then used to link the participant ID with participant information (think SQL database). 
...
Apache Arrow format instead of CSV files. The .parquet format is a cross-platform data.frame 
oriented format that can make it easier and quicker to move data between programming languages.  
...
package which provides a light weight reader/writer for parquet files. The 
[fst](https://www.fstpackage.org/) R package is another interesting option for serializing 
data.frmaes in R.
...
Sparse matrices can be saved and accessed eficiently using the [Matrix](https://matrix.r-forge.r-project.org/) 
package in R. And if you are accessing the same data often, consider storing as a 
[SQLite](https://www.sqlite.org/index.html) or [duckdb](https://duckdb.org/) database and accessing 
with [dbplyr](https://dbplyr.tidyverse.org/) or [sqlalchemy](https://www.sqlalchemy.org/) in Python. 
The main point is don't force data into a format that you're familiar with only because you're 
familiar with that format. This will often lead to large file sizes and inefficient performance.
...
```{.html}
20260515_my-descriptive-project-name/
├── data
├── doc
├── README.md
├── result
...
- a README file is present at the top level of the directory with a short 
...
If using R/Positron/Rstudio, include an `.Rproj` file at the top level of your directory. 
...
for more consistent workflows, use a workflow manager like [Nextflow](https://training.nextflow.io/), 
[Snakemake](https://snakemake.readthedocs.io/en/stable/), 
[WDL](https://openwdl.org/), or good ole' [GNU Make](https://swcarpentry.github.io/make-novice/)
...
Which is easier to read? Which is easier to debug? Which is more efficient? Which has a subtle 
error that you didn't notice and now feel the need to go back and look for?
...
R, the [air](https://tidyverse.org/blog/2025/02/air/) formatter can quickly clean up poorly 
formatted code. If using Python, [black](https://black.readthedocs.io/en/stable/index.html) is a 
highly opinionated formatter that is pretty popular. Although, [ruff](https://docs.astral.sh/ruff/) 
is currently all the rage with the Pythonistas these days. 
...
Another useful R idiom is to set up your output directories as variables at the top of your script
or .qmd document. I will often create a new script with the following at the top of the script. 
The "01" subdirectories should match the script name. For example, name the script 
"01_analysis.qmd" and all of the output will be nicely organized in subdirectories. Repeating this 
strategy will allow you to create as many new analysis scripts as you need and keep them organized.
...
datafiles <- here("results", "data-files", "01")
figures <- here("results", "figures", "01")
rdsfiles <- here("results", "rds-files", "01")

invisible(sapply(c(datafiles, figures, rdsfiles), dir.create, showWarnings = F, recursive = T))
```

Creating output directories at the top of the script like this allows you to save files and data 
using variable names instead of specifying entire paths like so:

```{.r}
# Saving a .png figure to the figures diectory
ggplot(...) + geom_point(...)
ggsave(here(figures, "scatterplot.png"))

# Saving a csv file to the "data-files" directory
fwrite(dt, here(datafiles, "dataframe.csv"))
```
